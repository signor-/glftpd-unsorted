...
AFFILS="XXXXX"
PRE_DIRS="/site/FLAC/_PRE/XXXXXX"
...
# check to see if -FLAC- is in the dirname, if not, deny MKD
if [ -z "`echo "$1" | egrep -i "\-FLAC\-"`" ]; then
	echo ".--------------------------------------------------."
	echo "| + Release is not a FLAC release, denied MKD!     |"
	echo ".--------------------------------------------------."
	logexit $2/$1 "Release is not a FLAC release, no -FLAC- in dirname"
fi
