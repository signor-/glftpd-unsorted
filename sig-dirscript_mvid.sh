...
ALLOWINTERNAL="0"
ALLOWINTGRPS="MVO_INT SRP_INT mV4U_iNT"
BANNEDGRP="iLUV"
BANNEDSOURCE=""
BANNEDLANG=""
...
# check to see if -x264- is in the dirname, if not, deny MKD
if [ -z "`echo "$1" | egrep -i "\-x264\-"`" ]; then
	if [ ! -z "`echo "$1" | egrep -i "sample"`" ]; then
		echo ".--------------------------------------------------."
		echo "| + Sample dir ok!                                 |"
		echo ".--------------------------------------------------."
	else
		echo ".--------------------------------------------------."
		echo "| + Release is not a x264 MVID release, denied MKD!|"
		echo ".--------------------------------------------------."
		logexit $2/$1 "Release is not a MVID release, no -x264- in dirname"
	fi
fi
...
echo $1 | grep -Ei "([0-9][0-9][0-9][0-9][-_.][0-9][0-9][-_.][0-9][0-9])" > /dev/null && {
