...
last_special_top_uploaders=`tail -$(($special_top_num*$tot_sec)) $back_path/toplastuploaders`
...
for sect in $site_sec; do
	...
	for uwt in $special_top_uploaders; do
		...
		echo "$week - $sec_name - $suser_rank - $suser_name with ${suser_file}f ${suser_megb}mb - ${suser_flag}${special_flag} flags" >> $special_log
	...
	echo "$week priveledged users for section $sec_name are -> $suser_allu"
done
