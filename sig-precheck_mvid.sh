...
VERIFY_MKV_WITH_SFV="TRUE"
VERIFY_RAR_WITH_SFV="TRUE"

ALLOWED="\.r[a|0-9][r|0-9]$ \.0[0-9][0-9]$ \.sfv \.nfo$ \.jpg$ \.mkv$"
...
DENY_WHEN_NO_SFV="\.mkv$ \.r[a|0-9][r|0-9]$ \.0[0-9][0-9]$"

EXCLUDEDDIRS="/REQUESTS/ /ARCHIVE/ /ARCHIVE2/ /PRIVATE/ /MVID/_PRE/ /MVID/.*/.*/SAMPLE /STATS/ /SPEEDTEST /RULES /SORTED/"

#chroot environment variable
VERBOSE=$(cat /bin/sig-verbose.txt)  

#--[ Script Start ]--------------------------------#

if [ "$VERBOSE" == "YES" ]; then
        echo -e ".--------------------------------------------------."
        echo -e "|             FILE CHECK IN PROGRESS...            |"
        echo -e ".--------------------------------------------------."
fi
...
if [ "$DENY_SFV_IN_SAMPLE_DIRS" = "TRUE" ]; then
	if [ "`echo "$PWD" | grep -i "\/sample$"`" ]; then
		if [ "`echo "$1" | grep "\.[sS][fF][vV]$"`" ]; then
			echo -e ".--------------------------------------------------."
			echo -e "| + File .sfv in sample dir denied!                |"
			echo -e ".--------------------------------------------------."
			exit 2
		fi
	fi
