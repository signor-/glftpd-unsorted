...
# DONT TOUCH THESE! SCRIPT HANDLES IT NOW!
AFFILS="XXX"
PRE_DIRS="/site/MP3/_PRE/XXX"
...
# check to see if -FLAC- is in the dirname, if it is, deny MKD, mp3 only fuckers!
if [ ! -z "`echo "$1" | egrep -i "\-FLAC\-"`" ]; then
	echo ".--------------------------------------------------."
	echo "| + Release is a FLAC release, denied MKD!         |"
	echo ".--------------------------------------------------."
	logexit $2/$1 "Release is a FLAC release, no -FLAC- in mp3 section"
fi
...
[ "$ALLOWINTERNAL" = "0" ] && {
	echo $1 | grep -Ei "[-](`echo $ALLOWINTGRPS | sed 's/ /|/g'`)$" > /dev/null; } || {
...
for grp in $BANNEDGRP; do
	echo $1 | egrep -i "[-](${grp}$|${grp}_INT$|${grp}[xX]$|${grp}[xX]_INT$|${grp}[-_]WEB$|${grp}[-_]WEB_INT$)" > /dev/null && {
        echo ".--------------------------------------------------."
...
echo $1 | grep -Ei "([0-9][0-9][-_.][0-9][0-9][-_.][0-9][0-9])" > /dev/null && {
